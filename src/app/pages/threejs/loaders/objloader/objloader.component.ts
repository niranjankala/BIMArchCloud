...
@Component({
  selector: 'bac-threejs-objloader',
  styleUrls: ['./objloader.component.scss'],
  templateUrl: './objloader.component.html',
})
export class ThreeJsObjLoaderComponent implements AfterViewInit {
  ...
  private createScene() {
      ...
      //this.scene.add(new AxisHelper(200));
  ...
  private onMTLLoadingCompleted(materials) {
      materials.preload();
      var loader = new OBJLoader();
  ...
  private onModelLoadingCompleted(collada) {
      ...
      var box = new THREE.Box3().setFromObject(collada);
      box.setFromCenterAndSize(collada.position, box.getSize(collada.position));
      collada.localToWorld(box);
      collada.position.multiplyScalar(-1);

      // var pos = collada.position;
      // this.camera.position.set(pos.x, pos.y, 10);
      // this.camera.lookAt(pos);
      // var boxFrmScene = new THREE.Box3().setFromObject(collada);
      // var height = Math.max(boxFrmScene.getSize(pos).y, boxFrmScene.getSize(pos).x);
